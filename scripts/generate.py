...
def main():
    ...
    t = args.targets
    dr = args.dry_run
    def do(tpl, out, **extra):
        if not dr: render(tpl, out, masterplan=mp, **extra)
        else: print(f"   [DRY-RUN] {out}")
    if "all" in t or "claude_md" in t:
        do("CLAUDE.md.j2", ROOT/"CLAUDE.md")
    if "all" in t or "agents_md" in t:
        do("AGENTS.md.j2", ROOT/"AGENTS.md")
    if "all" in t or "crews" in t:
        do("agents.yaml.j2", ROOT/"crews"/"agents.yaml")
